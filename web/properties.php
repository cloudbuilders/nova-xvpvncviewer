...
function vm_uptime($start_time)
{
    ...
    $then = mktime($then['tm_hour'], $then['tm_min'], $then['tm_sec'],
...
function main()
{
	...
	foreach ($srs as $sr) {
		...
		$possible_dvds[$vdi_rec["uuid"]] = $sr_rec["name_label"] . ": $vdi_display";
		$vdicount++;
	    }
	}
    }
	...
	if (($mem = xenapi_vm_get_memory_actual($session, $vm->metrics)) > 0)
	    $vm->memory_total = sprintf("%.1f GB", $mem / (1024 * 1024 * 1024));
	if ($state == "running")
