...
function vmSetDetails(fullname, labelname, state, rights, operations, platform, osversion)
{
   ...
   switch (state) {
   case "Running":
       context = contextForRights(operations, "console,shutdown,poweroff,reboot,reset,suspend,migrate,properties", rights);
       ...
       break;
   case "Halted":
       context = contextForRights(operations, "boot,booton,bootrecovery,properties", rights);
...
function updateReply()
{
	...
	var updated = "Last updated " + timestamp.lastChild.nodeValue;
	document.getElementById("timestamp").lastChild.nodeValue = updated;
